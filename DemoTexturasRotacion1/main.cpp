...
int main(){
	...
	Dementia::Node* nodeCylinderLeft = sceneMgr->createNode(
		...
		XMVectorSet(-0.1f, 0.0f, 0.0f, 0.0f)		// Indiquem la rotacio del node en agle de 360º (X,Y,Z,W)
	);
	...
	center->addChild(nodeCylinderLeft);
	center->addChild(nodeCylinderRight);
	...
	while( window.run(NULL) ){
		...
		center->setLocalRotation(XMVectorSet(0.0f, rotation, 0.0f, 0.0f));
		// Cambien la rotacio amb els valors nous.
		nodeCylinderLeft->setLocalRotation(XMVectorSet(rotation*1.5f, 0.0f, 0.0f, 0.0f));
		nodeCylinderRight->setLocalRotation(XMVectorSet(rotation*1.5f, 0.0f, 0.0f, 0.0f));
